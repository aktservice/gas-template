...
/**
 * @description Google Sheets データアクセス層 (共通共有版)
 * @author yoshitaka
 * @export
...
export class SheetRepository<T extends Record<string, any>> implements IRepository<T> {
  ...
  constructor(sheetName: string) {
    ...
    if (!sheet) {
      throw new Error(`スプレッドシート '${sheetName}' が見つかりませんでした。`);
    }
    this.sheet = sheet;
    this.loadHeaders();
  }
  ...
  private loadHeaders(): void {
    ...
    const headerRow = this.sheet.getRange(1, 1, 1, lastCol);
  ...
  getAll(): T[] {
    ...
    const dataRange = this.sheet.getRange(2, 1, lastRow - 1, this.sheet.getLastColumn());
